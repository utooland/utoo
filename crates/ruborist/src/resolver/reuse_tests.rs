...
fn test_find_resolved_node_uses_final_identity_for_file_and_git_sources() {
    for (spec, resolved_source, other_source) in [
        ...
        for (name, version, source, can_reuse) in [
            ...
            assert_eq!(
                find_resolved_node(&graph, graph.root_index, "shared", spec, spec, &resolved),
                expected,
...
#[test]
fn resolved_overrides_reclaim_unused_slots_only_when_scopes_match() {
    for (overrides, record_target, still_reachable, can_reclaim) in [
        ...
        for final_spec in ["2.0.0", "latest"] {
            assert_eq!(
                find_resolved_node(&graph, consumer, "shared", "^1.0.0", final_spec, &resolved),
                expected,
                "final spec {final_spec}, overrides {overrides}, recorded {record_target}, reachable {still_reachable}",
            );
        }
        assert_eq!(
            find_identical_node(&graph, consumer, "shared", "^1.0.0", &resolved),
            expected,
...
fn invalidated_target_remains_reusable_when_final_resolution_matches() {
    ...
    assert_eq!(
        find_resolved_node(&graph, consumer, "shared", "^1.0.0", "latest", &resolved),
        ReuseResult::Reuse(shared),
    );
    assert_eq!(
        find_identical_node(&graph, consumer, "shared", "^1.0.0", &resolved),
...
#[test]
fn slot_replacement_uses_original_requirement_for_conditional_override_scope() {
    let (graph, consumer, shared) = create_shared_revalidation_graph(
        serde_json::json!({ "consumer": { "shared@^1.0.0": "2.0.0" } }),
        true,
    );
    let resolved = create_version_manifest("shared", "2.0.0");
    assert!(graph.is_invalidated_target(shared));
    assert!(!graph.reachable_nodes().contains(&shared));

    // The override applies to the original ^1 request only inside consumer.
    // Testing its final target 2.0 instead would miss that scope restriction.
    assert_eq!(
        find_resolved_node(&graph, consumer, "shared", "^1.0.0", "2.0.0", &resolved),
        ReuseResult::Install(consumer),
    );
    assert_eq!(
        find_identical_node(&graph, consumer, "shared", "^1.0.0", &resolved),
