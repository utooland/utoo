    ...
    R: EventReceiver + 'static,
{
    ...
    // registry edges. Done inline before the loop (one-time cost).
    ...
    // INLINE-SPAWN_BLOCKING variant (Plan B A/B): graph state is
    // owned by the main task and shipped into spawn_blocking
    // per-iteration for mutation. No channels, no long-lived
    // graph_worker. Per-iter dispatch overhead (~tens of us) trades
    // off against the channel architecture's parallelism — tested
    // here vs the channel + spawn_blocking variant on PR #2920 to
    // verify whether the channel rewrite was overkill.
    type GraphState = (DependencyGraph, EdgeTargets, HashSet<(String, String)>);
    let mut graph_state: Option<GraphState> = Some((graph, edge_targets, seen_specs));
    ...
    let mut fetch_count: u64 = 0;
    let mut settle_count: u64 = 0;
    let mut graph_stats = GraphWorkerStats::default();
    ...
    loop {
        ...
        // Pipeline early-start: PackageResolved drives download.
        if let Some(core_arc) = cache.get_version_manifest(&out.name, &out.primary_spec) {
            receiver.on_event(BuildEvent::PackageResolved((&*core_arc).into()));
        }

        // Sibling drain on the same real package.
        if out.fetched
            && let Some(siblings) = deferred_by_real_name.remove(&out.real_name)
            && let Some(raw) = body_cache.lock().get(&out.real_name).cloned()
        {
        ...
        // Inline graph mutation on the blocking pool. Owned graph
        // state is moved in / moved out per iteration.
        let (graph_in, edge_targets_in, seen_specs_in) = graph_state.take().unwrap();
        let cache_inner = cache.clone();
        let receiver_inner = Arc::clone(&receiver);
        let build_config_inner = build_config.clone();
        let event_name = out.name;

        let (graph_out, edge_targets_out, seen_specs_out, new_pending, processed, sum_graph_us) =
            ...
            .await
            .context("inline graph step join")?;

        graph_state = Some((graph_out, edge_targets_out, seen_specs_out));
        graph_stats.processed += processed;
        graph_stats.sum_graph_us += sum_graph_us;
        graph_stats.new_specs_emitted += new_pending.len();
        pending.extend(new_pending);
    }

    let (graph, edge_targets, _) = graph_state.take().unwrap();
    graph_stats.unresolved_remaining = edge_targets.len();
    ...
    R: EventReceiver + 'static,
{
    ...
    let sum_graph_us = graph_start.elapsed().as_micros() as u64;
    (graph, edge_targets, seen_specs, new_specs, 1, sum_graph_us)
}
