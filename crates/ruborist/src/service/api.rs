    ...
    R: EventReceiver,
{
    ...
    // 7. Create registry client with persistence backend. Resolver-owned
    // per-run cache is seeded through `BuildDepsConfig::warm_project_cache`.
    ...
    Ok(BuildDepsOutput {
        ...
        project_cache: manifest_cache.into_project_cache(),
    })
}
