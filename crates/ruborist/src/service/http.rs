...
///
/// GHA npmjs pcap showed bun spreading resolve traffic across ~4 Cloudflare
/// edge IPs while a single reqwest pool concentrated most requests on one IP.
/// Four pools keeps the model small but gives the resolver enough independent
/// keep-alive pools to fan out when npmjs/full-manifest concurrency is raised.
#[cfg(not(target_arch = "wasm32"))]
