...
use crate::{InitializeOptions, commands};

/// Run the complete `utoo` command-line application.
///
/// Embedders normally call the functions under [`crate::commands`] instead.
/// This entry point exists so the native binary stays a thin wrapper around
...
async fn async_main() -> Result<()> {
    ...
    if let Some(Commands::Completions { shell }) = cli.command {
        ...
        if invocation::json() {
            let script =
                tokio::task::spawn_blocking(move || crate::commands::completions::generate(shell))
                    .await
    ...
    if let Some(Commands::Install(args)) = &cli.command {
        commands::install::migrate_from(args.from).await?;
    }
    ...
    match cli.command {
        Some(Commands::Clean { pattern, yes }) => {
            ...
            commands::clean::run(&pattern, confirmation).await?;
            log_time_end(&format!("{pattern} cleaned"));
        }
        Some(Commands::Install(args)) => {
            commands::install::run(args, cli.legacy_peer_deps).await?;
        }
        Some(Commands::Uninstall {
            ...
            commands::uninstall::run(specs, workspace, ScriptPolicy::from(ignore_scripts)).await?;
        }
        Some(Commands::Rebuild) => {
            let cwd = std::env::current_dir()?;
            commands::rebuild::run(&cwd).await?;
            log_time_end("All packages rebuilt");
        }
        Some(Commands::Deps { workspace_only }) => {
            commands::deps::run(workspace_only).await?;
        }
        Some(Commands::Update(args)) => {
            commands::update::run(args, ScriptPolicy::Run).await?;
            log_time_end("All packages updated");
        }
        Some(Commands::List { package }) => {
            let cwd = std::env::current_dir()?;
            commands::list::run(&cwd, &package).await?;
        }
        Some(Commands::Execute { command, args }) => {
            commands::execute::run(&command, args).await?;
        }
        Some(Commands::Run {
            script,
            workspace,
        ...
        Some(Commands::View { package }) => {
            commands::view::run(&package).await?;
        }
        Some(Commands::Link { packages, prefix }) => {
            commands::link::run(packages, prefix).await?;
        }
        Some(Commands::Init { yes }) => {
            ...
            commands::init::run(mode, output, None).await?;
        ...
        Some(Commands::Pack { path, dry_run }) => {
            commands::pack::run(path, dry_run.into()).await?;
        ...
        Some(Commands::Ping { registry }) => {
            commands::ping::run(registry.as_deref()).await?;
        }
        Some(Commands::Login) => {
        ...
        None => match cli.script_name {
            ...
            None => {
                commands::install::current_project(ScriptPolicy::from(cli.ignore_scripts)).await?
            }
        },
