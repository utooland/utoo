use crate::cli::Cli;
use crate::util::config_file::ConfigResult;
...
pub fn print_help(service: &crate::service::config::ConfigService) -> ConfigResult<()> {
    let config_commands = service.get_available_commands()?;
    ...
    println!(
        "  Global config:   {}",
        service.config().get_global_config_path()?.display()
    );
    let local_path = service.config().get_local_config_path()?;
