...
pub async fn link_current_to_global(prefix: Option<&str>) -> Result<()> {
    ...
    let package_info =
        PackageInfo::from_path(&project_path).with_context(|| format!("Failed to parse package info from path: {}", project_path.display()))?;
    ...
    link(&project_path, &global_package_path)
        .with_context(|| format!("Failed to link local project to global package: {} -> {}", project_path.display(), global_package_path.display()))?;
    ...
    if package_info.has_bin_files() {
        ...
        package_info
            ...
            .await
            .with_context(|| format!("Failed to link binary files to global bin directory: {}", global_bin_dir.display()))?;
    }
...
pub async fn link_global_to_local(
    ...
    // Create package info from path
    let package_info =
        PackageInfo::from_path(&global_package_path).with_context(|| format!("Failed to parse package info from path: {}", global_package_path.display()))?;
    ...
    link(&global_package_path, &local_link_path)
        .with_context(|| format!("Failed to link global package to local: {} -> {}", global_package_path.display(), local_link_path.display()))?;

    log_verbose(&format!("link global package to local: {} -> {}", global_package_path.display(), project_path.display()));

    // If the package has binary files, also link them to target project bin directory
    if package_info.has_bin_files() {
        log_verbose(&format!("Linking binary files to project bin directory: {}", package_info.name));
        let bin_dir = project_path.join("node_modules/.bin");
        package_info
            .link_to_target(&bin_dir)
            .await
            .with_context(|| format!("Failed to link binary files to project bin directory: {}", bin_dir.display()))?;
    }
