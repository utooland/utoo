...
/// Default concurrency for manifest fetching
const DEFAULT_MANIFEST_CONCURRENCY: usize = 64;

/// Default concurrency for tarball downloading
const DEFAULT_DOWNLOAD_CONCURRENCY: usize = 100;
...
impl PreloadService {
    ...
    pub async fn preload(
        ...
        tracing::info!(
            "Starting preload with {} initial dependencies, package_lock_only={}",
            initial_deps.len(),
            package_lock_only,
        );

        // Create semaphores for concurrency control
        let manifest_semaphore = Arc::new(Semaphore::new(DEFAULT_MANIFEST_CONCURRENCY));
        let download_semaphore = Arc::new(Semaphore::new(DEFAULT_DOWNLOAD_CONCURRENCY));
