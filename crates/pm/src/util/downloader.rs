...
async fn try_unpack(bytes: &[u8], dest: &Path) -> Result<()> {
    ...
    if (archive.unpack(dest).await).is_err() {
        ...
        for entry in archive.entries()? {
            ...
            // Get original permissions from tar file header
            ...
            // Use original permissions from tar file, but ensure directories have execute permission
            let permissions = if full_path.is_dir() {
                0o755 // Directories always need execute permission
            } else {
                original_mode // Keep original file permissions
            };
