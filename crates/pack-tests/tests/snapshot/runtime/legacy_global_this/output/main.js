 ...
 */ function interopEsm(raw, ns, allowExportDefault) {
    ...
    for(var current = raw; (typeof current === 'object' || typeof current === 'function') && !LEAF_PROTOTYPES.includes(current); current = getProto(current)){
        var _iteratorNormalCompletion = true, _didIteratorError = false, _iteratorError = undefined;
        try {
...
function getRuntimeChunkBasePath() {
    var basePath = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : CHUNK_BASE_PATH;
...
function chunkUrlToPath(chunkUrl) {
    ...
    var runtimeBasePath = getRuntimeChunkBasePath(RUNTIME_CHUNK_BASE_PATH);
 ...
 */ function resolveFileUrl(modulePath) {
    ...
    return "file:///ROOT/".concat(modulePath.split('/').map(encodeURIComponent).join('/'));
}
 ...
 */ function getChunkRelativeUrl(chunkPath) {
    ...
    return "".concat(getRuntimeChunkBasePath(basePath)).concat(encodedPath).concat(ASSET_SUFFIX);
}
...
function getPathFromScript(chunkScript) {
    ...
    var src = decodeURIComponent(chunkUrl.replace(/[?#].*$/, ''));
    var runtimeBasePath = getRuntimeChunkBasePath(RUNTIME_CHUNK_BASE_PATH);
    var path = src.startsWith(runtimeBasePath) ? src.slice(runtimeBasePath.length) : src;
    if (path.startsWith('/')) {
        path = path.slice(1);
    }
    return path;
...
(function() {
   ...
   */ function doLoadChunk(sourceType, chunkUrl) {
        ...
        if (sourceType === SourceType.Runtime) {
            ...
            // Load it first
            if (typeof importScripts !== 'function') {
                var decodedChunkUrl1 = decodeURI(chunkUrl);
                var previousScripts = document.querySelectorAll('script[src="'.concat(chunkUrl, '"],script[src^="').concat(chunkUrl, '?"],script[src="').concat(decodedChunkUrl1, '"],script[src^="').concat(decodedChunkUrl1, '?"]'));
        ...
        if (typeof importScripts === 'function') {
            ...
            var decodedChunkUrl2 = decodeURI(chunkUrl);
            if (isCss(chunkUrl)) {
                var previousLinks1 = document.querySelectorAll('link[rel=stylesheet][href="'.concat(chunkUrl, '"],link[rel=stylesheet][href^="').concat(chunkUrl, '?"],link[rel=stylesheet][href="').concat(decodedChunkUrl2, '"],link[rel=stylesheet][href^="').concat(decodedChunkUrl2, '?"]'));
                ...
                var previousScripts1 = document.querySelectorAll('script[src="'.concat(chunkUrl, '"],script[src^="').concat(chunkUrl, '?"],script[src="').concat(decodedChunkUrl2, '"],script[src^="').concat(decodedChunkUrl2, '?"]'));
                if (previousScripts1.length > 0) {
                    var _iteratorNormalCompletion1 = true, _didIteratorError1 = false, _iteratorError1 = undefined;
                    try {
                        var _loop = function() {
                        ...
                        for(var _iterator1 = Array.from(previousScripts1)[Symbol.iterator](), _step1; !(_iteratorNormalCompletion1 = (_step1 = _iterator1.next()).done); _iteratorNormalCompletion1 = true)_loop();
                    } catch (err) {
                        _didIteratorError1 = true;
                        _iteratorError1 = err;
                    } finally{
                    ...
                    switch(_state.label){
                        case 0:
                            if (chunk != null) {
                                ...
                                resolver = getOrCreateResolver(getUrlFromScript(chunkPath));
                                resolver.resolve();
                            }
                            if (params == null) {
                                return [
