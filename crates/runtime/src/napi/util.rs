// N-API helper macros and utilities.
// Ported from Deno's ext/napi/util.rs.
...
pub(crate) extern "C" fn backing_store_deleter_callback(
    ...
    let mut finalizer = unsafe { Box::<BufferFinalizer>::from_raw(deleter_data as _) };
...
macro_rules! return_error_status_if_false {
    ($env:expr, $condition:expr, $status:ident) => {
        if !$condition {
            return Err($crate::napi::util::napi_set_last_error($env, $status).into());
        }
    };
}
...
pub(crate) unsafe fn check_new_from_utf8_len<'s>(
    ...
    return_error_status_if_false!(
        env,
        (len == NAPI_AUTO_LENGTH) || len <= libc::INT_MAX as _,
        napi_invalid_arg
    );
...
pub(crate) fn napi_set_last_error(env: *const Env, error_code: napi_status) -> napi_status {
    let env = unsafe { &*env };
