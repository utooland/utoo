...
use crate::napi::util::check_new_from_utf8;
use crate::napi::util::check_new_from_utf8_len;
use crate::napi::util::get_array_buffer_ptr;
use crate::napi::util::make_external_backing_store;
use crate::napi::util::napi_clear_last_error;
use crate::napi::util::napi_set_last_error;
use crate::napi::util::v8_name_from_property_descriptor;
use crate::napi::function::CallbackInfo;
use crate::napi::function::create_function;
use crate::napi::function::create_function_template;
use crate::napi::*;
use crate::check_env;
use crate::check_arg;
...
// --- Values: undefined, null, boolean, global, typeof ---
...
pub unsafe extern "C" fn napi_get_value_int32(
  ...
  let value = if let Ok(value) = value.try_cast::<v8::Int32>() {
    value.value()
  } else if let Some(value) = value.int32_value(scope) {
    value
  } else {
    return napi_set_last_error(env_ptr, napi_number_expected);
  };
...
pub unsafe extern "C" fn napi_get_value_uint32(
  ...
  let num = if let Ok(value) = value.try_cast::<v8::Uint32>() {
    value.value()
  } else if let Some(value) = value.uint32_value(scope) {
    value
  } else {
    return napi_set_last_error(env_ptr, napi_number_expected);
  };
...
pub unsafe extern "C" fn napi_get_value_string_utf8(
  ...
  if buf.is_null() {
    check_arg!(env, result);
    unsafe {
      *result = value.utf8_length(scope);
    }
  } else if bufsize != 0 {
    let buffer =
...
pub unsafe extern "C" fn napi_has_named_property(
  ...
  let Some(has_property) = object.has(scope, key.into()) else {
    return napi_generic_failure;
  };
...
pub unsafe extern "C" fn napi_create_function(
  ...
  let name = if !name.is_null() {
    match unsafe { check_new_from_utf8_len(env_ptr, name, length) } {
      Ok(s) => Some(s),
      Err(status) => return status,
    }
  } else {
...
pub unsafe extern "C" fn napi_define_class(
  ...
  let name = match unsafe { check_new_from_utf8_len(env_ptr, utf8name, length) } {
    Ok(string) => string,
    Err(status) => return status,
  };
  ...
  for p in napi_properties {
    ...
    if p.getter.is_some() || p.setter.is_some() {
      let getter = p
        .getter
        .map(|g| create_function_template(scope, env_ptr, None, g, p.data));
      let setter = p
        .setter
      ...
      let function =
        create_function_template(scope, env_ptr, None, method, p.data);
      let proto = tpl.prototype_template(scope);
...
pub unsafe extern "C" fn napi_instanceof(
  ...
  let Some(res) = object.unwrap().instance_of(scope, ctor) else {
    return napi_generic_failure;
  };
...
pub unsafe extern "C" fn napi_type_tag_object(
  ...
  let key = v8::Local::new(scope, &env.shared().type_tag);
  ...
  let Some(tag) = v8::BigInt::new_from_words(scope, false, slice) else {
    return napi_generic_failure;
  };

  if !object.set_private(scope, key, tag.into()).unwrap_or(false) {
    return napi_generic_failure;
  }
...
pub unsafe extern "C" fn napi_check_object_type_tag(
  ...
  let Some(val) = object.get_private(scope, key) else {
    return napi_generic_failure;
  };
...
pub unsafe extern "C" fn napi_escape_handle(
  ...
  // SAFETY: napi_value is pointer-sized; lifetime is phantom at FFI boundary.
  unsafe {
    *(result as *mut usize) = std::mem::transmute::<napi_value<'_>, usize>(escapee);
  }
...
pub unsafe extern "C" fn napi_get_reference_value(
  ...
  let value = match &reference.state {
    ...
    ReferenceState::Weak(w) => w.to_local(scope),
  };
...
pub unsafe extern "C" fn napi_create_typedarray(
  ...
  macro_rules! create {
    ($TypedArray:ident, $size_of_element:expr) => {{
      ...
      let Some(ta) =
        v8::$TypedArray::new(scope, ab, byte_offset, length)
      else {
        return napi_generic_failure;
      };
      ta.into()
...
pub unsafe extern "C" fn napi_create_dataview(
  ...
  // SAFETY: Erase lifetime to decouple input/output lifetimes at FFI boundary.
  let arraybuffer: napi_value<'static> = unsafe { std::mem::transmute(arraybuffer) };
  let Some(buffer) =
    arraybuffer.and_then(|v| v8::Local::<v8::ArrayBuffer>::try_from(v).ok())
  else {
    return napi_invalid_arg;
...
pub unsafe extern "C" fn napi_run_script(
  ...
  let Some(rv) = script.run(scope) else {
    return napi_generic_failure;
  };
