// libuv compatibility shims for utoo-runtime N-API support.
// Ported from Deno's ext/napi/uv.rs.
...
fn assert_ok(res: c_int) -> c_int {
  if res != 0 {
    ...
    // don't panic because that might unwind into c/c++
    std::process::abort();
  }
  res
}

// Platform-specific sizes matching libuv structs.
// macOS: pthread_mutex_t = 64 bytes
#[cfg(target_os = "macos")]
const UV_MUTEX_SIZE: usize = 64;
#[cfg(target_os = "linux")]
const UV_MUTEX_SIZE: usize = 40;
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn uv_mutex_init(lock: *mut uv_mutex_t) -> c_int {
  unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn uv_mutex_lock(lock: *mut uv_mutex_t) {
  unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn uv_mutex_unlock(lock: *mut uv_mutex_t) {
  unsafe {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn uv_mutex_destroy(_lock: *mut uv_mutex_t) {
  // no cleanup required
}
...
const UV_HANDLE_PADDING: usize = (UV_HANDLE_SIZE
  - size_of::<*mut c_void>()
...
const UV_ASYNC_PADDING: usize = (UV_ASYNC_SIZE
  - size_of::<*mut c_void>()
  - size_of::<*mut uv_loop_t>()
  - size_of::<uv_handle_type>()
  - size_of::<uv_async_cb>()
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn uv_async_send(handle: *mut uv_async_t) -> c_int {
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn uv_close(handle: *mut uv_handle_t, close: uv_close_cb) {
  unsafe {
    if handle.is_null() {
      close(handle);
      return;
